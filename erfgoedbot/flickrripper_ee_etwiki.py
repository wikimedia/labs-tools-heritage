...
# -*- coding: utf-8  -*-
'''
Tool to copy a flickr stream to Estonian Wikipedia 
...
ripper_config = {
    ...
    'categories': u'[[Kategooria:Flickrist kopeeritud pildid]]\n',
}
...
def findDuplicateImages(photo=None,
                        site=pywikibot.getSite(u'et', u'wikipedia')):
...
def getFilename(photoInfo=None, site=pywikibot.getSite(u'et', u'wikipedia'),
                project=u'Flickr'):
...
def getMonumentId(photoInfo):
    ...
    for tag in getTags(photoInfo):
        # print tag
        if prog.match(tag.lower()) <> None:
            return tag.replace(ripper_config['flickr_tagprefix'],"")
    return u'';
...
def buildDescription(flinfoDescription=u'', flickrreview=False, reviewer=u'',
    ...
    description = description.replace(u'{{flickrreview}}', u'{{KunstiteoseFoto}}')
