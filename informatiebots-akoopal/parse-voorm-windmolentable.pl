...
foreach my $provincie (@provincies) {
  $provincielijst = "Sjabloon:Tabeldata lijst van voormalige windmolens in $provincie";
...
EOF
  ...
  foreach my $record (@records) {
	...
	foreach $cat (@cats) {
      ...
      $paginauit = $paginauit."|-\n|$fields[0] || $fields[1] || $redirect || $dppagina || $fotolijst || $fotoartikel || $objectsjabloon || $molendb || $dhmlink || $coordinaten || $commonscat || ". join(", ", sort @statcats). " || ". join(", ", sort @loccats). " || ". join(", ", sort @typecats). " || ". join(", ", sort @funccats). " || ". join(", ", sort @misccats). "\n";
...
#  print $paginauit;
  ...
  $paginastatuit = $paginastatuit."|-\n|[[Gebruiker:Akoopal/Voormalige windmolens in $provincie|$provincie]] || $objectcount || $artikelencount ||".int($artikelencount*100/$objectcount+.5)."%|| $fotocount ||".int($fotocount*100/$objectcount+.5)."%|| $fotoartcount||".int($fotoartcount*100/$objectcount+.5)."%|| $coordinatencount ||".int($coordinatencount*100/$objectcount+.5)."%\n";
}
...
$editor->edit("Gebruiker:Akoopal/Voormalige windmolenoverzicht",$paginastatuit,"Autorefreshed");
