...
my @provincies = ("Belgi\x{EB}", "Nederland");
...
foreach my $provincie (@provincies) {
  $provincielijst = "Lijst van vuurtorens in $provincie";
...
EOF
  ...
  foreach my $record (@records) {
	...
	# Infobox in artikel?
	$objectsjabloon = checkinfobox($object, 'vuurtoren');
	...
	foreach $cat (@cats) {
	  $cat =~ s/_/ /g;
	  if ( $cat =~ m/vuurtoren in/i ) { push @loccats, $cat; }
#	  elsif ( $cat =~ m/vaardig/i ) { push @statcats, $cat; }
  ...
  $paginauit = "[[Gebruiker:Akoopal/Vuurtorenoverzicht|Overzicht]]<br>\n'''$provincie'''\n*Lijst: [[$provincielijst]]\n* Aantal vuurtorens in lijst: $objectcount\n* Aantal artikelen: $artikelencount\n* Aantal foto's in de lijst: $fotocount\n* Aantal foto's in een artikel: $fotoartcount\n".$paginauit."|}\n";
  # print $paginauit;
  $editor->edit("Gebruiker:Akoopal/Vuurtorens in $provincie",$paginauit,"Autorefreshed");
  ...
  print "$provincie heeft $objectcount vuurtorens, $artikelencount artikelen, $fotoartcount foto's in de artikelen en $fotocount foto's in de lijst\n";

  $paginastatuit = $paginastatuit."|-\n|[[Gebruiker:Akoopal/Vuurtorens in $provincie|$provincie]] || $objectcount || $artikelencount ||".int($artikelencount*100/$objectcount+.5)."%|| $fotocount ||".int($fotocount*100/$objectcount+.5)."%|| $fotoartcount||".int($fotoartcount*100/$objectcount+.5)."%|| $coordinatencount ||".int($coordinatencount*100/$objectcount+.5)."%\n";
}
...
# print $paginastatuit;
$editor->edit("Gebruiker:Akoopal/Vuurtorenoverzicht",$paginastatuit,"Autorefreshed");
