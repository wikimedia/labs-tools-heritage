...
my @provincies = ("Overijssel", "Gelderland", "Noord-Brabant", "Limburg" );
...
EOF
  ...
  foreach my $record (@records) {
	...
	foreach $cat (@cats) {
	  ...
	  elsif ( $cat =~ m/bovenslag|middenslag|onderslag|middenslag|turbine/i ) { push @typecats, $cat; }
...
#  print $paginauit;
  ...
  $paginastatuit = $paginastatuit."|-\n|[[Gebruiker:Akoopal/Watermolens in $provincie|$provincie]] || $objectcount || $artikelencount ||".int($artikelencount*100/$objectcount+.5)."%|| $fotocount ||".int($fotocount*100/$objectcount+.5)."%|| $fotoartcount||".int($fotoartcount*100/$objectcount+.5)."%|| $coordinatencount ||".int($coordinatencount*100/$objectcount+.5)."%\n";
}
...
$editor->edit("Gebruiker:Akoopal/Watermolenoverzicht",$paginastatuit,"Autorefreshed");
